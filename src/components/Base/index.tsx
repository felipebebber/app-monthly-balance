import { useEffect, useReducer, useState, useMemo, useCallback } from 'react';

import STORAGE_KEYS from '../../data/storageKeys';

import Block from '../Block';
import Modal from '../Modal';
import FormExpenses from '../FormExpenses';
import Chart from '../Chart';
import Balance from '../Balance';
import FormEdit from '../FormExpenses/Form';
import DateControlWrapper from './DateController';

import FormContext from '../../context/FormContext';
import TableContext from '../../context/TableContext';
import GlobalContext from '../../context/GlobalContext';

import ExpensesReducer from '../../hooks/useExpensesReducer';
import RemoveType from '../../types/RemoveType';

const initialValue = {}
const getDateParts = (d = new Date()) => ({
  month: d.getMonth().toString(),
  year: d.getFullYear().toString(),
});

function Base() {
    const date = new Date();

    const [currentMonth, setCurrentMonth] = useState(getDateParts().month);
    const [currentYear, setCurrentYear] = useState(getDateParts().year);
    const [expenses, updateExpenses] = useReducer(ExpensesReducer, initialValue);
    const [modalConfig, setModalConfig] = useState({});
    const [modalVisible, setModalVisible] = useState(false);

    useEffect(() => {
        const storedExpenses = localStorage.getItem(STORAGE_KEYS.EXPENSES);
        const storedYear = localStorage.getItem(STORAGE_KEYS.YEAR);
        const storedMonth = localStorage.getItem(STORAGE_KEYS.MONTH);
        
        if (storedExpenses) {
            updateExpenses({type: 'set', expenses: JSON.parse(storedExpenses)});
        }
        
        if (storedMonth && storedYear) {
            setCurrentMonth(storedMonth);
            setCurrentYear(storedYear);
        }
    }, []);

    useEffect(() => {
        const storedYear = localStorage.getItem(STORAGE_KEYS.YEAR);
        const storedMonth = localStorage.getItem(STORAGE_KEYS.MONTH);

        if (storedMonth && storedMonth !== currentMonth) {
            setCurrentMonth(storedMonth);
        }
        
        if (storedYear && storedYear !== currentYear) {
            setCurrentYear(storedYear);
        }
        
    }, [expenses]);

    const currentList = useMemo(() => {
        return expenses?.[currentYear]?.[currentMonth] || [];
    }, [expenses, currentYear, currentMonth]);

    const dispatchExpense = useCallback((type, payload) => updateExpenses({ type, ...payload }), []);

    const modal = useCallback((config) => setModalConfig(config), []);

    const removeExpenseModal = useCallback(
        ({ id, ...rest }: RemoveType) => {
            if (!Number.isInteger(id)) return;
            modal({
                type: 'confirm',
                text: <b>Deseja remover despesa {id}?</b>,
                callback: () => { 
                    dispatchExpense('remove', { id, ...rest })   
                }
            });
        }, []
    );
    
    const editExpenseModal = useCallback(
        function(expense, id) {
            modal({
                title: `Edição de despesa ${id}`,
                html: <FormEdit type="full" 
                    fn={(exp) => dispatchExpense('edit', { expense: exp, id })} 
                    values={expense} 
                    key={`${expense.month}${expense.year}${id}`} 
                    callback={() => setModalVisible(false)}
                />
            });
        },
        []
    )
    
    const resetExpenseList = function() {
        if (!Object.keys(expenses).length) return;
        localStorage.removeItem(STORAGE_KEYS.EXPENSES);
        localStorage.removeItem(STORAGE_KEYS.YEAR);
        localStorage.removeItem(STORAGE_KEYS.MONTH);

        updateExpenses({type: 'set', expenses: {}});
        
        const { month, year } = getDateParts();
        setCurrentMonth(month);
        setCurrentYear(year);
    }
    
    return (
        <GlobalContext.Provider value={{setModalVisible, modalVisible, modalConfig}}>
            <div className="bg-gray-50 text-gray-800 h-lvh md:flex gap-4">
                <div className="grid gap-4 md:m-auto w-full max-w-[800px] shadow-sm p-4 rounded-md grid-cols-1 bg-white">
                    <div>
                        <div className='font-bold text-center text-3xl md:text-4xl text-white w-fit m-auto p-2 pt-1 pb-1 bg-blue-600 uppercase rounded'>Balanço Mensal</div>
                        <DateControlWrapper values={{expenses, currentMonth, currentYear, setCurrentMonth, setCurrentYear}} />
                    </div>
                    <hr className="border-0 border-t border-gray-200 my-2"/>
                    <div className='flex flex-col gap-4'>
                        <div className="w-full mx-auto">
                            <FormContext.Provider value={(exp) => dispatchExpense('add', { expense: exp })}>
                                <FormExpenses />
                            </FormContext.Provider>
                        </div>
                        <div className='flex md:flex-row flex-col-reverse gap-4 m-auto w-full md:h-[400px]'>
                            <div className="flex-4/12">
                                <Block title="Chart" className="h-full">
                                    <Chart list={currentList} />
                                </Block>
                            </div>
                            <div className="flex-8/12">
                                <Block title="Balanço" className="h-full">
                                    <TableContext.Provider value={{editExpenseModal, removeExpenseModal}}>
                                        <Balance list={currentList} />
                                    </TableContext.Provider>
                                </Block>
                            </div>
                        </div>
                        <div className='text-right'>
                            <button className='border-red-300 border px-2 py-1 rounded text-sm cursor-pointer hover:bg-red-300' onClick={() =>
                                modal({
                                    type: 'confirm',
                                    text: <b>Deseja deletar todas despesas?</b>,
                                    callback: resetExpenseList,
                                })
                            }>
                                Deletar Storage
                            </button>
                        </div>
                    </div>
                </div>
            </div>
            <Modal />
        </GlobalContext.Provider>
    )
};


export default Base;
    